...
describe('getComputed', () => {
    ...
    it('respects simple overrides', () => {
        expect(
        // @ts-expect-error intentionally not using a full EUI theme definition
        getComputed(theme, buildTheme({ sizes: { small: 4 } }, ''), 'LIGHT')).toEqual({
            colors: { primary: '#000', secondary: '#000000' },
            sizes: { small: 4 },
            themeName: 'minimal',
        });
    ...
    it('respects overrides in computation', () => {
        expect(getComputed(
        // @ts-expect-error intentionally not using a full EUI theme definition
        theme, buildTheme({ colors: { LIGHT: { primary: '#CCC' } } }, ''), 'LIGHT')).toEqual({
    ...
    it('respects property extensions', () => {
        expect(getComputed(
        // @ts-expect-error intentionally not using a full EUI theme definition
        theme, buildTheme({ colors: { LIGHT: { tertiary: '#333' } } }, ''), 'LIGHT')).toEqual({
    ...
    it('respects section extensions', () => {
        expect(getComputed(
        // @ts-expect-error intentionally not using a full EUI theme definition
        theme, buildTheme({ custom: { myProp: '#333' } }, ''), 'LIGHT')).toEqual({
            colors: { primary: '#000', secondary: '#000000' },
            sizes: { small: 8 },
            custom: { myProp: '#333' },
            themeName: 'minimal',
    ...
    it('respects extensions in computation', () => {
        ...
        theme, buildTheme({
            colors: {
                LIGHT: {
                    tertiary: computed(([primary]) => `${primary}333`, ['colors.primary']),
                },
            },
        }, ''), 'LIGHT')).toEqual({
...
describe('mergeDeep', () => {
    ...
    it('merge complex objects, second into first', () => {
        expect(mergeDeep({ a: 1, b: { c: { d: 3 } } }, { b: { c: { d: 4 } } })).toEqual({ a: 1, b: { c: { d: 4 } } });
        expect(mergeDeep({ a: 1, b: { c: { d: 3 } } }, { b: { c: { e: 5 } } })).toEqual({ a: 1, b: { c: { d: 3, e: 5 } } });
    });
});
describe('getTokenName', () => {
    it('returns the correct token name', () => {
        expect(getTokenName('backgroundBase', 'primary')).toEqual('backgroundBasePrimary');
